...
crc_async_impl::check(pmt::pmt_t msg)
{
  ...
  case crc::CRC16_CCITT:
    crc16_c = crc::crc16_ccitt(pdu, pdu_len - 2);
    memcpy(&crc16_received, pdu + pdu_len - 2, 2);
  ...
  case crc::CRC16_AUG_CCITT:
    crc16_c = crc::crc16_aug_ccitt(pdu, pdu_len - 2);
    memcpy(&crc16_received, pdu + pdu_len - 2, 2);
    crc16_received = ntohs(crc16_received);
    if (crc16_c != crc16_received) {
      return;
  ...
  case crc::CRC16_CCITT_REVERSED:
    crc16_c = crc::crc16_ccitt_reversed(pdu, pdu_len - 2);
    memcpy(&crc16_received, pdu + pdu_len - 2, 2);
    crc16_received = ntohs(crc16_received);
    if (crc16_c != crc16_received) {
      return;
    ...
    break;
  case crc::CRC16_IBM:
    crc16_c = crc::crc16_ibm(pdu, pdu_len - 2);
    memcpy(&crc16_received, pdu  + pdu_len - 2, 2);
    crc16_received = ntohs(crc16_received);
    if (crc16_c != crc16_received) {
      return;
    ...
    break;
  case crc::CRC32_C:
    crc32_c = crc::crc32_c(pdu, pdu_len - 4);
    memcpy(&crc32_received, pdu + pdu_len - 4, 4);
    ...
    break;
  default:
    throw std::runtime_error("crc_async: Invalid CRC");
    return;
  }
