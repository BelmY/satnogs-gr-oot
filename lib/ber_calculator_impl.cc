...
ber_calculator_impl::create_pdu()
{
  ...
  d_cnt++;
  uint32_t crc = crc::crc32_c(d.data(), d_frame_size - sizeof(uint32_t));
  memcpy(d.data() + d_frame_size - sizeof(uint32_t), &crc, sizeof(uint32_t));
  whitening::whitening_sptr w = whitening::make_ccsds(true);
...
ber_calculator_impl::received_pdu(pmt::pmt_t msg)
{
  ...
  uint32_t crc = crc::crc32_c(d, d_frame_size - sizeof(uint32_t));
  uint32_t crc_recv = 0;
  memcpy(&crc_recv, d + d_frame_size - sizeof(uint32_t), sizeof(uint32_t));
  if (crc != crc_recv) {
    d_inval_cnt++;
    return;
  }
